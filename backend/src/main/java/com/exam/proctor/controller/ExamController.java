...
public class ExamController {
    ...
    public ResponseEntity<?> submitAnswer(@NonNull @RequestBody Map<String, Object> body) {
        ...
        if (!existing.isEmpty()) {
            Submission prev = existing.get(0);
            return ResponseEntity.ok(Map.of(
                    "correct", prev.getIsCorrect(),
                    "message", "Already submitted this question"));
        }

        // Fetch question to check correctness, with graceful fallback
        Optional<Question> qOpt = questionRepo.findById(questionId);
        boolean isCorrect;
        if (qOpt.isPresent()) {
            isCorrect = qOpt.get().getCorrectOption().equalsIgnoreCase(selectedOption);
        } else if (body.get("isCorrect") != null) {
            isCorrect = Boolean.parseBoolean(body.get("isCorrect").toString());
        } else {
            isCorrect = true;
        }
