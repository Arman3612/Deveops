...
public class SubmissionWorkerService {
    ...
    public static class RabbitSubmissionListener {

        @Autowired
        private SubmissionWorkerService workerService;

        @RabbitListener(queues = RabbitMQConfig.SUBMISSION_QUEUE)
        public void onMessage(AnswerDTO answer) {
            workerService.consumeSubmission(answer);
        }
    }
}
